...
forest_pred <- lapply(fit_forest@ensemble, function(x)
    unlist(party:::R_getpredictions(x, party:::R_get_nodeID(x, object@inputs, 0))))
forest_pred <- as.data.frame(do.call("cbind", forest_pred))
forest_pred <- forest_pred[, sample(1:ncol(forest_pred), 25)]
