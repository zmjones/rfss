...
ggsave('figures/o.png', p, width = 20, height = 12, units = "cm")
...
p <- p + geom_point(size = 5) + scale_shape_manual(values=c("D", "R"))
ggsave('figures/p1.png', p, width = 20, height = 12, units = "cm")
...
p <- p + geom_vline(aes(xintercept = sp1))
p <- p + geom_point(size = 5) + scale_shape_manual(values=c("D", "R"))
ggsave('figures/p2.png', p, width = 20, height = 12, units = "cm")

png("figures/cart.png", width = 30, height = 18, units = "cm", res = 500)
plot(fit_cart,inner_panel=node_inner(fit_cart,pval=F)
     ,terminal_panel=node_barplot(fit_cart, beside=F,id=F))
dev.off()
